...
UINT16 time_LCD_Display=0;
UINT32 lcd_DATA_Packet_ID=0;
u8 lcd_DATA_Packet_Control=0;
u8 lcd_length_Struct_DATA_Packet_Contro=0;
u8 lcd_Struct_DATA_Packet_Contro[9]={0};
UINT8  flag_lcd_id_updata=0;
u8 FLAG_KEY_SW4_close=0;
u8 FLAG_KEY_SW3_stop=0;
u8 FLAG_KEY_SW2_open=0;



//#define Factory_DEF
/******************************************************************************
 * FunctionName: lcd_GPIO_init
 *
 * Function Description: This function initializes display GPIO.
 *
...
/*************************************************/
//LCDœ‘ æ(L)*(H 8µƒ±∂ ˝)£¨¥Æ––ƒ£ Ω
//Àµ√˜£∫x,y¥˙±Ì◊¯±Í£ªL∫ÕH¥˙±ÌÕºœÒ¥Û–°
/*************************************************/
...
void lcd_clear(unsigned char data1) 
{
    ...
    unsigned char seg;
   unsigned char page;
   for(page=0xb0;page<0xb9;page++) //D°‰®∞3¶Ã??°§128 ®∞3 0xb0----0xb8
    {
      send_command(page);
...
void lcd_clear_1and2_line(unsigned char data1) 
{
    ...
    unsigned char seg;
   unsigned char page;
   for(page=0xb0;page<0xb2;page++) //D°‰®∞3¶Ã??°§128 ®∞3 0xb0----0xb8
    {
      send_command(page);
      send_command(0x10); //®¢D¶Ã??°§°Í???¶Ã®™°¡??®≤®¢?°‰?D°‰®®?°Í?°‰®Æ¶Ã®≤0 ®¢D?a®∫?
      send_command(0x00);
...
void lcd_clear_3and8_line(unsigned char data1) 
{
    ...
    unsigned char seg;
   unsigned char page;
   for(page=0xb2;page<0xb9;page++) //D°‰®∞3¶Ã??°§128 ®∞3 0xb0----0xb8
    {
      send_command(page);
      send_command(0x10); //®¢D¶Ã??°§°Í???¶Ã®™°¡??®≤®¢?°‰?D°‰®®?°Í?°‰®Æ¶Ã®≤0 ®¢D?a®∫?
      send_command(0x00);
...
void lcd_clear_line1(unsigned char data1) 
{
    ...
    unsigned char seg;
   unsigned char page;
   for(page=0xb0;page<0xb1;page++) //D°‰®∞3¶Ã??°§128 ®∞3 0xb0----0xb8
    {
      send_command(page);
      send_command(0x10); //®¢D¶Ã??°§°Í???¶Ã®™°¡??®≤®¢?°‰?D°‰®®?°Í?°‰®Æ¶Ã®≤0 ®¢D?a®∫?
      send_command(0x00);
...
void lcd_clear_line8(unsigned char data1) 
{
    ...
    unsigned char seg;
   unsigned char page;
   for(page=0xb7;page<0xb8;page++) //D°‰®∞3¶Ã??°§128 ®∞3 0xb0----0xb8
    {
      send_command(page);
      send_command(0x10); //®¢D¶Ã??°§°Í???¶Ã®™°¡??®≤®¢?°‰?D°‰®®?°Í?°‰®Æ¶Ã®≤0 ®¢D?a®∫?
      send_command(0x00);
...
void lcd_desplay(void)
{
  ...
  char rssi;
    ...
    //if(time_LCD_Display)Display_key_SW3=0xff;
    //if((Display_key_SW3!=Count_key_SW3)&&(time_LCD_Display==0))
	...
	{  
      ...
      LCD_display_argos_rssi(rssi*10);
      lcd_rssi= rssi;
...
*/
	...
	for(i=0;i<2;i++)
    display_map_xy(0+i*6,0,5,8,char_Small+(char_rssi1[i]-0x20)*5);     //RSSI

	for(i=0;i<3;i++)
    display_map_xy(86+24+i*6,0,5,8,char_Small+(char_rssi[i]-0x20)*5);     //dBm
...
/*--  Œƒ◊÷:  I  --*/
/*--  ÀŒÃÂ18;  ¥À◊÷ÃÂœ¬∂‘”¶µƒµ„’ÛŒ™£∫øÌx∏ﬂ=12x24   --*/
...
const unsigned char char_company[]={                                               //π´Àæ
/*--  Œƒ◊÷:  Œƒ  --*/
/*--  ÀŒÃÂ18;  ¥À◊÷ÃÂœ¬∂‘”¶µƒµ„’ÛŒ™£∫øÌx∏ﬂ=24x24   --*/
...
const unsigned char char_Medium[]={
  ...
  0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,                //42          «Â∆¡
...
/*--  µ˜»Î¡À“ª∑˘ÕºœÒ£∫D:\My Documents\π§◊˜◊ ¡œ\»’±æŒƒªØ\ŒƒªØ∏ƒ–Õ\TCXO∞Ê LCDÕºœÒ ˝æ›\NULL.bmp  --*/
/*--  øÌ∂»x∏ﬂ∂»=16x16  --*/
...
/*--  µ˜»Î¡À“ª∑˘ÕºœÒ£∫D:\My Documents\π§◊˜◊ ¡œ\»’±æŒƒªØ\ŒƒªØ∏ƒ–Õ\TCXO∞Ê LCDÕºœÒ ˝æ›\o.bmp  --*/
/*--  øÌ∂»x∏ﬂ∂»=16x16  --*/
