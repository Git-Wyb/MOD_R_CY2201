...
u8 ADF7030_GET_FW_STATE(void)
{
    ...
    DELAY_30U();
    ADF7030_READ_REGISTER_NOPOINTER_LONGADDR(ADDR_MISC_FW, 6); //??7030????
    state = (u8)(ADF7030_RESIGER_VALUE_READ >> 8) & 0x3F;
    return state;
}
...
{
    u8 cache;
    ADF7030_GET_FW_STATE();
    ADF7030_WRITE_REGISTER_NOPOINTER_LONGADDR_MSB(ADDR_CRMGT_PROC_CLK_EN, 0xFFFFFFFF);
    WaitForADF7030_FIXED_DATA(); //等待芯片空闲/可接受CMD状态
    DELAY_30U();
    ADF7030_WRITE_REGISTER_NOPOINTER_LONGADDR_MSB(ADDR_PMU_KEY, 0x00000001);
    WaitForADF7030_FIXED_DATA(); //等待芯片空闲/可接受CMD状态
    DELAY_30U();
    while (GET_STATUE_BYTE().CMD_READY == 0)
        ;
    ADF7030_CHANGE_STATE(DO_PHY_RESET);
    do
    {
        GET_STATUE_BYTE();
        while (GET_STATUE_BYTE().CMD_READY == 0)
